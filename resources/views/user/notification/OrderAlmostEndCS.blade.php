...
<a href="{{ route('pemesanan.index', ['lapangan' => $notification->data['lapangan']['id']]) }}" class="notify-item">
    <div class="notify-thumb"><i class="ti-basketball btn-warning"></i></div>
    <div class="notify-text">
        <p>Pemesanan di Lapangan {{ $notification->data['lapangan']['name'] }} - {{ $notification->data['olahraga']['name'] }} pada {{ $time->format('H:i') }} - {{ $time1->format('H:i') }} {{ App\Lapangan::find($notification->data['lapangan']['id'])->zone }} Akan segera berakir dalam waktu @if($notification->created_at->minute == 0) 1 Jam @else {{ 60 - $notification->created_at->minute }} Menit @endif</p>
