...
<a href="{{ route('PO.pembayaran.show', ['order' => $notification->data['order']['id']]) }}" class="notify-item">
    ...
    <div class="notify-text">
        <p>Pembayaranmu untuk pemesanan di Lapangan {{ $notification->data['lapangan']['name'] }} - {{ $notification->data['olahraga']['name'] }} ({{ $time->format('H:i') }} - {{ $time1->format('H:i') }} {{ App\Lapangan::find($notification->data['lapangan']['id'])->zone }}) telah diterima</p>
