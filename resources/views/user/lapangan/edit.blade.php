...
@section('content')
    @form(['method' => 'POST', 'route' =>route('lapangan.update', ['lapangan' => $lapangan]), 'fileupload' => 'yes'])
        ...
        @slot('body')
            ...
            <div class="form-gp @isset($lapangan->no_telepon) focused @endisset mb-1">
                <label for="no_telepon">Nomor Telepon</label>
                <input type="text" pattern="^(^\+62\s?|^0)(\d{3,4}-?){2}\d{3,4}$" name="no_telepon" value="{{ $lapangan->no_telepon ?? '' }}">
                <i class="fa fa-phone"></i>
            </div>
            <div class="form-group @isset($lapangan->no_telepon) focused @endisset">
                ...
                <select class="form-control form-control-lg" name="jenis_rekening" required>
                    @foreach(App\Rekening::$jenisRekening as $jenis_rekening)
                        <option @if($lapangan->rekeningTerbaru->jenis_rekening == $jenis_rekening) selected @endif>{{ $jenis_rekening }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-gp @isset($lapangan->rekening) focused @endisset">
                <label for="rekening">Rekening*</label>
                <input type="text" name="rekening" value="{{ $lapangan->rekeningTerbaru->rekening }}">
                <i class="fa fa-money"></i>
            </div>
            <div class="form-gp @isset($lapangan->rekening_atas_nama) focused @endisset">
                <label for="atas_nama">Atas Nama Pada Rekening*</label>
                <input type="text" name="atas_nama" value="{{ $lapangan->rekeningTerbaru->rekening_atas_nama }}" required autofocus>
                <i class="ti-user"></i>
            </div>
            <div class="table-responsive">
                <table class="table borderless">
                    ...
                    <tr>
                        ...
                        <td class="py-0 px-0">
                            <div class="custom-control custom-checkbox custom-control-inline">
                                <input type="checkbox" @if($lapangan->hari(0)) checked @endif class="custom-control-input" id="minggu" name="hari_buka[]" value="0">
            ...
            <div class="table-responsive">
                <table class="table borderless">
                    <tr>
                        <td class="px-0 py-0" style="color:#7e74ff">Ubah Gambar Lapangan</td>
                    </tr>
                    <tr>
